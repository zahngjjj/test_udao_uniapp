<template>
  <view class="container">
          ...
          <view class="copy-card">
            ...
            <view class="task-info">
              <view class="info-row">
                ...
                <text class="info-value">{{ item.startUser?.nickname || '未知' }}</text>
              </view>
              ...
              <view class="info-row" v-if="item.reason">
                ...
                <text class="info-label">抄送原因：</text>
              ...
              <view class="info-row" v-if="item.id">
                ...
                <text class="info-label">抄送编号：</text>
    ...
    <!-- 空状态 -->
    <u-empty 
      v-if="copyList.length === 0 && !loading"
      text="暂无抄送记录"
      icon="copy"
    ></u-empty>
  </view>
...
const getCopyList = async (isRefresh = false) => {
  ...
  try {
    ...
    if (response && response.code === 0) {
      ...
      // 更新加载状态
      if (copyList.value.length >= total.value) {
...
const handleViewDetail = (item) => {
  ...
  uni.navigateTo({
    url: `/pages/flow/task/detail?processInstanceId=${item.processInstanceId}&activityId=${item.activityId}&readonly=true`
  })
}
