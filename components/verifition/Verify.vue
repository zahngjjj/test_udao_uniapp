<template>
  <view :class="props.mode=='pop'?'mask':''" v-show="showBox">
    <view :class="props.mode=='pop'?'verifybox':''" :style="{'max-width':parseInt(props.imgSize.width)+30+'px'}">
      ...
      <view class="verifybox-bottom" :style="{padding:props.mode=='pop'?'15px':'0'}">
        <!-- 验证码容器 -->
        <!-- 滑动 -->
        <view v-if="componentType=='VerifySlide'">
          <VerifySlide @success="success" :captchaType="props.captchaType" :type="verifyType" :figure="props.figure"
                       :arith="props.arith" :mode="props.mode" :vSpace="props.vSpace" :explain="props.explain" :imgSize="props.imgSize"
                       :blockSize="props.blockSize" :barSize="props.barSize" :defaultImg="defaultImg" ref="instance"
                       @changeClickShow="handleChangeClickShow"></VerifySlide>
        </view>
        <!-- 点选 -->
...
import {ref, onMounted, onUnmounted, defineProps, defineEmits, computed,watch, defineExpose} from 'vue'
...
// const defaultImg = ref(require('@/static/images/default.jpg'))
...
//======提供给子组件调用=========
const handleChangeClickShow = (val) => {
  clickShow.value = val
}
