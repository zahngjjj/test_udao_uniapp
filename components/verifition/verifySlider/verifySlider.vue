<template>
  <view style="position: relative;">
    <view v-if="props.type === '2'" class="verify-img-out" :style="{height: (parseInt(props.imgSize.height) + props.vSpace) + 'px'}">
      <view class="verify-img-panel" :style="{width: props.imgSize.width,
                                                   height: props.imgSize.height,}">

        <image :src="state.backImgBase?('data:image/png;base64,'+state.backImgBase):props.defaultImg" alt=""
               style="width:100%;height:100%;display:block"></image>
        <view class="verify-refresh" @click="refresh" v-show="state.showRefresh">
          <text class="iconfont icon-refresh"></text>
        </view>
        <transition name="tips">
          <text class="verify-tips" v-if="state.tipWords" :class="state.passFalg ? 'suc-bg':'err-bg'">{{state.tipWords}}</text>
        </transition>
      </view>
    </view>

    <!-- 公共部分 -->
    <view class="verify-bar-area" :style="{width: props.imgSize.width,
                                              height: '40px',
                                              'line-height':'40px'}">
      <text class="verify-msg" v-text="state.text"></text>
      <view class="verify-left-bar"
            :style="{width: state.leftBarWidth?state.leftBarWidth:'40px', height: '40px', 'border-color': state.leftBarBorderColor, transaction: state.transitionWidth}">
        <text class="verify-msg" v-text="state.finishText"></text>
        <view class="verify-move-block" @touchstart="start" @touchend="end" @touchmove="move"
              :style="{width:'40px', height: '40px', 'background-color': state.moveBlockBackgroundColor, left: state.moveBlockLeft, transition: state.transitionLeft}">
          <text :class="['verify-icon iconfont', state.iconClass]" :style="{color: state.iconColor}"></text>
          <view v-if="props.type === '2'" class="verify-sub-block" :style="{'width':Math.floor(parseInt(imgSize.width)*47/310)+ 'px' ,
                                  'height': props.imgSize.height,
                                  'top':'-' + (parseInt(props.imgSize.height) + props.vSpace) + 'px',
                                  }">

            <image :src="'data:image/png;base64,'+state.blockBackImgBase" alt=""
                   style="width:100%;height:100%;display:block"></image>
          </view>
        </view>
      </view>
...
import {ref,defineProps,nextTick,getCurrentInstance,watch } from "vue";
/**
 * VerifySlide
 * @description 滑块
 * */
...
//定义emit
const emit = defineEmits(['success', 'error', 'ready','changeClickShow'])
...
const init = () => {
  state.value.text = state.value.explain
  getPicture()
  nextTick(()=>{
    // 调用ready方法  这行代码好像没有什么意思
    // emit('ready',this)
  })
}
...
//鼠标按下
const start = (e) =>{
  state.value.startMoveTime = new Date().getTime(); //开始滑动的时间
  if (state.value.isEnd == false) {
    state.value.text = ''
    state.value.moveBlockBackgroundColor = '#337ab7'
    state.value.leftBarBorderColor = '#337AB7'
    state.value.iconColor = '#fff'
    e.stopPropagation();
...
//鼠标移动
const move = (e) => {
  var query = uni.createSelectorQuery().in(getCurrentInstance());
  ...
  barArea.boundingClientRect(data => {
    ...
    if (state.value.status && state.value.isEnd == false) {
      if (!e.touches) { //兼容移动端
      ...
      if (state.value.type !== '1') { //图片滑动
        if (move_block_left >= barArea_offsetWidth - parseInt(parseInt(props.blockSize
            .width) / 2) - 2) {
          move_block_left = barArea_offsetWidth - parseInt(parseInt(props.blockSize
              .width) / 2) - 2;
        }
      }
      ...
      //拖动后小方块的left值
      state.value.moveBlockLeft = (move_block_left - parseInt(parseInt(props.blockSize.width) / 2)) +
          "px"
      state.value.leftBarWidth = (move_block_left - parseInt(parseInt(props.blockSize.width) / 2)) +
          "px"
...
const end=()=>{
  ...
  state.value.endMovetime = new Date().getTime();
  var _this = this;
  //                判断是否重合
  if (state.value.status && state.value.isEnd == false) {
    if (props.type !== '1') { //图片滑动
      var moveLeftDistance = parseInt((state.value.moveBlockLeft || '').replace('px', ''));

      moveLeftDistance = moveLeftDistance * 310 / parseInt(props.imgSize.width)

      var captchaVerification = state.value.secretKey ? aesEncrypt(state.value.backToken + '---' + JSON.stringify({
        x: moveLeftDistance,
        y: 5.0
      }), state.value.secretKey) : state.value.backToken + '---' + JSON.stringify({
        x: moveLeftDistance,
        y: 5.0
      })
      let data = {
        captchaType: state.value.captchaType,
        "pointJson": state.value.secretKey ? aesEncrypt(JSON.stringify({
          x: moveLeftDistance,
          y: 5.0
        }), state.value.secretKey) : JSON.stringify({
          x: moveLeftDistance,
          y: 5.0
        }),
        "token": state.value.backToken
      }
      myRequest({
        // url: `/captcha/check`,
        ...
        let res = result.data
        if (res.repCode == "0000") {
          state.value.moveBlockBackgroundColor = '#5cb85c'
          state.value.leftBarBorderColor = '#5cb85c'
          state.value.iconColor = '#fff'
          state.value.iconClass = 'icon-check'
          state.value.showRefresh = true
          state.value.isEnd = true;
          setTimeout(() => {
            if (props.mode == 'pop') {
              //父级组件的clickShow改为false
              emit('changeClickShow',false)
            }
            refresh();
          }, 1500)
          state.value.passFalg = true
          state.value.tipWords =
              `${((state.value.endMovetime-state.value.startMoveTime)/1000).toFixed(2)}s验证成功`
          setTimeout(() => {
            state.value.tipWords = ""
            // 调用emit方法
          ...
          state.value.moveBlockBackgroundColor = '#d9534f'
          state.value.leftBarBorderColor = '#d9534f'
          state.value.iconColor = '#fff'
          state.value.iconClass = 'icon-close'
          state.value.passFalg = false
          setTimeout(() => {
            refresh();
          }, 1000);
          emit('error', getCurrentInstance)
          emit.tipWords = "验证失败"
          setTimeout(() => {
            emit.tipWords = ""
          }, 1000)
...
const refresh = () => {
  ...
  state.value.showRefresh = true
  state.value.finishText = ''
  state.value.transitionLeft = 'left .3s'
  state.value.moveBlockLeft = 0
  state.value.leftBarWidth = false
  state.value.transitionWidth = 'width .3s'
  state.value.leftBarBorderColor = '#ddd'
  state.value.moveBlockBackgroundColor = '#fff'
  state.value.iconColor = '#000'
  state.value.iconClass = 'icon-right'
  getPicture()
  state.value.isEnd = false
  setTimeout(() => {
    state.value.transitionWidth = ''
    state.value.transitionLeft = ''
    state.value.text = props.explain
  }, 300)
}
...
const getPicture = () => {
  ...
  myRequest({
    ...
    let res = result.data
    if (res.repCode == "0000") {
      state.value.backImgBase = res.repData.originalImageBase64
      state.value.blockBackImgBase = res.repData.jigsawImageBase64
      state.value.backToken = res.repData.token
      state.value.secretKey = res.repData.secretKey

      console.log("获取验证码成功")
    }
    // 判断接口请求次数是否失效
    if (res.repCode == '6201') {
      state.value.backImgBase = null
      state.value.blockBackImgBase = null
    }
  })
}
...
// 不知道这里为啥没有生效，暂时注释掉，在onLoad的时候进行初始化
